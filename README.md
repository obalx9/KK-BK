...
| `JWT_SECRET` | Случайная строка не менее 64 символов для подписи JWT токенов |

### Необязательные (для медиа)

| Переменная | Описание |
|-----------|----------|
| `S3_ENDPOINT` | Endpoint S3 (например `https://s3.twcstorage.ru`) |
| `S3_REGION` | Регион S3 (например `ru-1`) |
| `S3_BUCKET` | Имя бакета |
| `S3_ACCESS_KEY` | Access Key для S3 |
| `S3_SECRET_KEY` | Secret Key для S3 |

**Важно:** Если S3 переменные не заданы, сервер запустится, но `/api/media/upload` будет возвращать ошибку 503.

### Необязательные (для OAuth)

| Переменная | Описание |
|-----------|----------|
| `APP_URL` | URL фронтенда для CORS (по умолчанию `*`) |
